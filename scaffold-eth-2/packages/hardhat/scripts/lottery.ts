...
async function bet(index: string, amount: string) {
  ...
  const allowTx = await token.connect(accounts[Number(index)]).approve(contractAddress, ethers.MaxUint256);
  await allowTx.wait();
...
async function claimPrize(index: string, amount: string) {
  const tx = await contract.connect(accounts[Number(index)]).prizeWithdraw(ethers.parseUnits(amount));
  const receipt = await tx.wait();
...
async function burnTokens(index: string, amount: string) {
  ...
  const tx = await contract.connect(accounts[Number(index)]).returnTokens(ethers.parseUnits(amount));
  const receipt = await tx.wait();
